...
from euxrvsh_core.config import resolve_storage_root
...
class EuxrvshAstrBotPlugin(Star):
    def __init__(self, context: Context, config: AstrBotConfig | None = None):
        ...
        default_storage_root = self._default_storage_root()
        storage_root, sqlite_path_compat_used = resolve_storage_root(
            self.config,
            default_storage_root=str(default_storage_root),
        )
        check_result = run_startup_check(str(storage_root), sqlite_path_compat_used=sqlite_path_compat_used)
        ...
        self.runtime = build_runtime(self.config, default_storage_root=str(default_storage_root))
        self.app = PvpApplication(self.runtime.battle_service)
        logger.info("Euxrvsh PvP 初始化完成，storage_root：%s", self.runtime.config.storage_root)
        logger.info("Euxrvsh PvP runtime.db：%s", self.runtime.config.runtime_db_path)
        logger.info("Euxrvsh PvP builtin 角色目录：%s", self.runtime.config.builtin_roles_dir)
        logger.info("Euxrvsh PvP custom 角色目录：%s", self.runtime.config.custom_roles_dir)
        if self.runtime.config.sqlite_path_compat_used:
            logger.warning("Euxrvsh PvP 当前通过兼容字段 sqlite_path 推导 storage_root，建议迁移到 storage_root 配置。")
        for warning in self.runtime.role_warnings:
            logger.warning("Euxrvsh PvP 角色加载警告：%s", warning)
    ...
    async def llm_use_skill(self, event: AstrMessageEvent, skill_key: str, target_slot: int = 0):
        """使用技能。
        适用意图：用户明确要放技能、点名某一个技能、技能命中某一个目标。
    ...
    def _default_storage_root(self) -> Path:
        ...
        return base_path / "plugin_data" / plugin_name
